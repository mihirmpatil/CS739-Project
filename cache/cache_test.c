...
void* allocAndRead(void *args){
  ...
  // Populate the memory in 1KB chunks
...
int main(int argc, char *argv[]){
  ...
  if(argc != 5){
    printf("Usage: <executable> <number of threads> <chunksize in kb> <operation> <number of runs>\nOperation:\n1 - Write once and read repeatedly\n2 - Write continuously\n3 - Mix both read and write\n");
    return 0;
  }
  ...
  totalsize = atoi(argv[2])*1024;
  runs = atoi(argv[4]);
  ops = atoi(argv[3]);
  chunksize = atoi(argv[2]);
  
  //printf("Starting thread:%d chunksize:%d runs:%d ops:%d\n",threads,totalsize/(1024),runs,ops);
